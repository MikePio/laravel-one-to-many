...
<div class="container overflow-auto p-5 d-flex flex-column align-items-center" style="max-height: calc(100vh - 70.24px);">
  ...
  <form action="{{ route('adminprojects.store') }}" method="POST" enctype="multipart/form-data">
    ...
    @error('image')
      <p class="text-danger">{{ $message }}</p>
    @enderror
    <div class="mb-3" style="width: 150vh; max-width: 73vw;">        <label for="category" class="form-label">Category</label>
    ...
    @error('category')
      <p class="text-danger">{{ $message }}</p>
    @enderror
    <div class="mb-3" style="width: 150vh; max-width: 73vw;">        <label for="start_date" class="form-label">Start date</label>
